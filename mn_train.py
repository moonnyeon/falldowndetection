...
data_files = ['../Data/scalepose/Home_new-set(labelXscrw).pkl']
...
#######################################################
    ...
    for e in range(epochs):
        if training_A:
            ...
            with tqdm(dataloader['train']) as iterator:
                ...
                # in two frames.
                    ...
                    pts = pts[:, :2, :, :]
                    a = model3(pts)
                    pts_auto = pts.permute(0, 3, 1, 2).contiguous()
                    pts_auto = pts_auto.view(pts_auto.size(0), pts_auto.size(1), -1)

                    loss_nll = nll_gaussian(pts_auto, a, variance=5e-3)
                    #print('loss_nll_mn',loss_nll_mn.data.item())
                    #loss_auto = criterion(a,pts_auto)
                    #print('loss',loss_auto.data.item())
            ...
            if((e%10) ==0 ):
                mn_save_model(model=model3,name = 'epoch{}_model3.pt'.format(e))
                    
            print('Epoch: {:04d}'.format(e),'nll_train: {:.10f}'.format(np.mean(nll_train)))        
