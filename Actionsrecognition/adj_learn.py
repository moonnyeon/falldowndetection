...
class InteractionDecoderRecurrent(nn.Module):
	...
	def single_step_forward(self, inputs, rel_rec, rel_send, rel_type, hidden):
		...
		#print('gg')
		tem = self.input_r(inputs)
		#print('gg1')
		#print(np.shape(self.input_r(inputs))) #30
		
		#print('gg2')
		#print(np.shape(self.hidden_r(agg_msgs))) #60
...
class AdjacencyLearn(nn.Module):
	...
	def __init__(self, n_in_enc, n_hid_enc, edge_types, n_in_dec, n_hid_dec, node_num=14):
		super().__init__()
	...
	def forward(self, inputs): # [N, 3, 50, 25, 2]
		...
		N, C, T, V = inputs.size()
		x = inputs.permute(0, 3, 1, 2).contiguous() # [N, 2, 25, 3, 50]
		# N,V,C,T
		x = x.contiguous().view(N, V, C, T).permute(0,1,3,2)  # [2N, 25, 50, 3]  ### ###torch.Size([32, 14, 30, 3]) (my data)
		#x = x.contiguous().view(N, V, C, T) #torch.Size([32, 14, 3, 30]) (my data)
		...
		for i in range(self.N):
			...
			for j in range(1, self.c):
				A = torch.sparse.FloatTensor(self.offdiag_indices, self.edges[i,:,j], torch.Size([14, 14])).to_dense().cuda(gpu_id)
				A = A + torch.eye(14, 14).cuda(gpu_id)
