...
class GraphConvolution(nn.Module):
    ...
    def forward(self, x, A):
        #print('check0',np.shape(x))
        x = self.conv(x)
        #print('check1',np.shape(x))
        n, kc, t, v = x.size()
        #print('check2',np.shape(x))
        x = x.view(n, self.kernel_size, kc//self.kernel_size, t, v)
        #print('check3',np.shape(x))
        x = torch.einsum('nkctv,kvw->nctw', (x, A))
        #print('check4',np.shape(x))
        return x.contiguous()
...
class st_gcn(nn.Module):
    ...
    def forward(self, x, A):
        ...
        x = self.gcn(x, A)
        #print('after gcn  shape',np.shape(x))
        x = self.tcn(x) + res
...
class TwoStreamSpatialTemporalGraph(nn.Module):
    ...
    def __init__(self, graph_args, num_class, edge_importance_weighting=True,
                 **kwargs):
        super().__init__()
        self.pts_stream = StreamSpatialTemporalGraph(2, graph_args, None,
